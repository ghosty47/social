...
const dpUpload = async (req, res) => {
    ...
    if(!req.file) return res.status(404).json({msg: 'Please upload an image'})
    // if(req.file.mimetype != 'image/*') return res.status.json({msg: 'Only Images are allowed'})
    ...
    if(!uploadedImage) return res.status(404).json({msg: 'An error occurred while uploading'})
